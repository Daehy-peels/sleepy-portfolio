        ...
        <div className="hidden md:flex items-center space-x-4">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.path}
              // --- REVAMPED CLASSES FOR VISIBILITY, PRETTIER BUTTONS & FIXED UNDERLINE ---
              className={`
                group // Essential for group-hover effects
                text-lg py-2 px-4 rounded-full relative inline-flex items-center justify-center // Basic button styling
                ...
                after:absolute after:bottom-0 after:left-1/2 after:h-[3px] after:w-0 // Starts centered, zero width
                ...
                after:transform after:-translate-x-1/2 // Correctly centers the underline
                after:opacity-0 // Initially completely transparent (fixes unwanted visible lines)

                // Underline Visibility based on Active/Hover State
                ${
                  location.pathname === link.path
                    ? "after:w-full after:opacity-100" // Active: always full width and visible
                    : "group-hover:after:w-full group-hover:after:opacity-100" // Inactive: hidden, expands and becomes visible on group hover
                }
              `}
            >
              {link.name}
