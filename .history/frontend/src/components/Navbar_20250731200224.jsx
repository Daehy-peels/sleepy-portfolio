...
import {
  ...
  FaLinkedin,
  FaBars,
  FaTimes, // <-- Corrected: Import FaTimes instead of X
  FaTwitter,
} from "react-icons/fa";
  ...
  return (
    ...
    <motion.nav
      ...
      className="fixed top-0 left-0 right-0 z-50
                 bg-white/50 dark:bg-gray-900/50       // Adjusted opacity to 50% for more transparency
                 backdrop-blur-lg backdrop-saturate-150 // Using 'lg' blur and added saturate for liquid effect
                 border-b border-blue-100/50 dark:border-blue-900/30 // Slightly more transparent border
                 shadow-sm transition-colors duration-300" // Kept shadow for depth
      style={{ WebkitBackdropFilter: "blur(16px) saturate(150%)" }} // For Safari compatibility, added saturate
    >
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-2 lg:gap-6">
          ...
          <motion.button
            onClick={toggleTheme}
            className="ml-4 p-2 rounded-full border border-blue-300 dark:border-blue-700
                       bg-white/60 dark:bg-gray-900/60 shadow-md backdrop-blur-lg transition-colors duration-300"
        ...
        <div className="md:hidden flex items-center space-x-4">
          ...
          <motion.button
            onClick={toggleTheme}
            className="p-2 rounded-full border border-blue-300 dark:border-blue-700
                       bg-white/60 dark:bg-gray-900/60 shadow-md backdrop-blur-lg transition-colors duration-300"
          ...
          <motion.button
            className="relative p-2 rounded-full border border-blue-200 dark:border-blue-800
                       bg-white/60 dark:bg-gray-900/60 shadow-sm backdrop-blur-lg transition-colors duration-300 flex items-center justify-center overflow-hidden"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            animate="visible"
            exit="exit"
            // The mobile menu overlay itself uses a blurred gradient,
            // which is good for the "liquid glass" effect.
            className="fixed inset-0 z-40
                       bg-gradient-to-br from-blue-100/80 via-white/90 to-blue-200/80
                       dark:from-blue-950/90 dark:via-gray-900/95 dark:to-blue-900/80
                       backdrop-blur-2xl flex flex-col items-center justify-center md:hidden"
          >
            {/* Close button for mobile menu */}
            <motion.button
              className="absolute top-6 right-6 p-3 rounded-full border border-blue-200 dark:border-blue-800
            ...
            <motion.button
              onClick={toggleTheme}
              className="mt-12 p-3 rounded-full border border-blue-300 dark:border-blue-700
                         bg-white/60 dark:bg-gray-900/60 shadow-md"
              whileHover={{ scale: 1.15, rotate: 180 }}
              whileTap={{ scale: 0.93 }}
              transition={{ type: "spring", stiffness: 300, damping: 20 }}
