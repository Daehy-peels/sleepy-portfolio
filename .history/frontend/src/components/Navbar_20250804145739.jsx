...
const Navbar = ({ theme, toggleTheme }) => {
  ...
  // Effect to handle scroll and set active link
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <motion.nav
      ...
      className="fixed top-0 left-0 right-0 z-50
                 bg-white/50 dark:bg-gray-900/50 /* Semi-transparent background, no blur */
                 border-b border-blue-100/50 dark:border-blue-900/30
                 shadow-sm transition-colors duration-300"
      // Removed style={{ WebkitBackdropFilter: "..." }}
    >
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        {/* Desktop Nav Links and Social Icons */}
        <nav className="hidden md:flex items-center gap-2 lg:gap-6">
          <ul className="flex gap-2 lg:gap-6">
            {navLinks.map((link) => (
              ...
              >
                    ...
                    className={`uppercase tracking-widest text-sm font-semibold px-2 py-1 rounded transition-colors duration-200 no-underline focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400
                      ${
                        activeLink === link.href
                          ? "text-blue-600 dark:text-blue-400 font-extrabold" // Active link styling
                ...
                {activeLink === link.href && (
                  <motion.div
                    layoutId="active-link" // Unique ID for layout animations
          ...
          >
            <motion.span
              key={theme} // Key to trigger re-render and animation on theme change
              initial={{
                rotate: theme === "dark" ? -360 : 360,
                opacity: 0,
              }} /* Spin from different directions */
        ...
        <div className="flex items-center md:hidden">
          ...
          >
            <motion.span
              key={theme} // Key to trigger re-render and animation on theme change
              initial={{ rotate: theme === "dark" ? -360 : 360, opacity: 0 }}
              animate={{ rotate: 0, opacity: 1 }}
              transition={{
                duration: 0.5,
                type: "spring",
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            animate="visible"
            exit="exit"
            // Removed backdrop-blur-2xl for no blur effect in mobile menu too
            className="fixed inset-0 z-40 bg-gradient-to-br from-blue-100/90 via-white/95 to-blue-200/90 dark:from-blue-950/95 dark:via-gray-900/98 dark:to-blue-900/95 flex flex-col items-center justify-center md:hidden"
          >
            {/* Close Button */}
            <motion.button
              onClick={toggleMenu}
                  ...
                  <motion.li key={link.name} variants={menuItemVariants}>
                    <a
                      ...
                      className={`text-2xl font-bold uppercase tracking-widest transition-colors duration-300 no-underline focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400
                        ${
                          activeLink === link.href
                            ? "text-blue-700 dark:text-blue-300" // Active link styling for mobile
