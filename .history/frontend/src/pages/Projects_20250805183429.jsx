...
const Projects = () => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        // Fetch projects from your local backend API
        ...
        setError(
          "Failed to fetch projects. Please ensure the backend server is running."
        );
      } finally {
        setIsLoading(false);
      }
    ...
    >
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-12">
        {projects.map((project) => (
          <motion.div
            key={project._id} // Use the unique MongoDB ID as the key
            initial={{ opacity: 0, y: 50 }}
            whileInView={{ opacity: 1, y: 0 }}
